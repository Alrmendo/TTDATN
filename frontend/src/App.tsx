...
import { Product, Category, AuthUser, Customer, Invoice, Promotion, Store, PurchaseOrder, ApiStore } from './types';
...
export default function App() {
  ...
  // Danh sách chi nhánh — khởi tạo bằng mock data (initialStores) để UI có gì đó hiển thị
  // ngay lập tức, sau đó loadStores() sẽ thay bằng dữ liệu thật (UUID thật) từ /api/stores.
  // Bắt buộc phải là UUID thật vì các API khác (vd. GET/PUT /api/inventory) dùng storeId
  // này để truy vấn PostgreSQL — mock id kiểu "CH001" sẽ gây lỗi "invalid input syntax for type uuid".
  ...
  const loadStores = async () => {
	try {
	  ...
	  if (!res.ok) return; // giữ nguyên initialStores (mock) làm fallback nếu API lỗi
	  ...
	  setStores(
		data.map((s) => ({
		  ...
		  address: s.address,
		  phone: s.phone ?? '',
		  managerName: '', // ApiStore không trả field này; các màn dùng `stores` hiện không hiển thị nó
		}))
	  );
	} catch (err) {
	  console.error(err); // giữ nguyên initialStores (mock) làm fallback nếu mất kết nối
	}
  };
			...
			<main className="flex-grow p-6 md:p-8 space-y-6 overflow-y-auto max-w-7xl w-full mx-auto" id="workspace-content">
			  ...
			  {userRole === 'Quản lý' && (
				<>
				  ...
				  {activeTab === 'Tài khoản' && (
					<AccountManagement stores={stores} />
				  )}
				  {activeTab === 'Báo cáo' && (
					<RevenueReport />
				  )}
