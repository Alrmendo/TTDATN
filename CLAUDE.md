...
| `purchase-order.model.ts` | `purchase_orders` | `status` ENUM('pending','debt','completed','cancelled') — **`'debt'` thêm tuần 5** (commit "Thêm nợ nhà cung cấp, xuất pdf hóa đơn"), xem state machine bên dưới; `confirmedBy`/`confirmedAt` nullable cho tới khi `confirmReceipt()`; `totalCost` tính trong `PurchaseOrderService.createPurchaseOrder`/`confirmReceipt` (không phải instance method `calculateTotalCost()` như Schema.md mục 7 mô tả — lệch vị trí đặt logic, không lệch kết quả; **Schema.md cũng chưa được cập nhật cho `'debt'`**, xem "What Still Needs to Be Built") |
...
- `order.routes.ts` → `POST /api/invoices`, `POST /api/invoices/:id/items`, `DELETE /api/invoices/:id/items/:productId`, `POST /api/invoices/:id/promotion`, `POST /api/invoices/:id/promotion/auto` (**mới tuần 5** — auto chọn KM tốt nhất, gọi bởi `SalesManagement.tsx` sau mỗi lần sửa giỏ hàng), `POST /api/invoices/:id/confirm-payment` (all Staff only), `GET /api/invoices` (Staff, Manager)
...
- `PurchaseOrderService.ts` — `createPurchaseOrder` (transaction: tạo order + bulk-create details, `totalCost` tính từ `items`), `getPurchaseOrders` (filter `storeId`/`status`/`startDate`/`endDate`/`search` — search dùng `unaccent()` trên tên NCC, xem Known issues tuần 2 #8), `getPurchaseOrderById`, `confirmReceipt` (transaction: update từng `receivedQuantity` + gọi `InventoryService.updateInventory(..., 'increase')` dùng chung, không viết riêng; tính lại `totalCost` từ `receivedQuantity` thực nhận — **tuần 5:** nếu `totalCost <= 0` (vd toàn bộ dòng nhận 0) thì set `status='completed'` thẳng, ngược lại `status='debt'` — tránh đơn kẹt vĩnh viễn ở `'debt'` vì `recordPayment` yêu cầu `amount > 0` nên không có đường tới `'completed'` nếu cứ set `'debt'` cho đơn nợ 0 đồng), `cancelOrder` (chỉ huỷ khi `status='pending'`) — **state machine đầy đủ 4 trạng thái:** `pending` → (`confirmReceipt`) → `debt` hoặc thẳng `completed` nếu không có gì để nợ; `pending` → (`cancelOrder`, chỉ từ `pending`) → `cancelled`; `debt` → (`PurchaseOrderPaymentService.recordPayment`, trả đủ) → `completed`
...
2. **`PO_STATUS_LABEL`/`PurchaseOrderStatus` (`types.ts`) chưa được dùng ở đâu** — thêm cùng lúc với commit "PurchaseOrder state machine" nhưng `WarehouseManagement.tsx` (nơi hiển thị status đơn nhập hàng) vẫn tự hardcode label riêng, chưa import 2 export mới này. **Cập nhật:** sau commit "Thêm nợ nhà cung cấp" (thêm status `'debt'`), type này ở `types.ts` càng lệch hơn — vẫn chỉ khai báo 3 trạng thái (`'pending'|'completed'|'cancelled'`), thiếu `'debt'`, trong khi `WarehouseManagement.tsx STATUS_MAP` cục bộ đã có đủ 4.
...
5. **`Schema.md` chưa cập nhật theo commit "Thêm nợ nhà cung cấp, xuất pdf hóa đơn"** — §7 `purchase_orders` vẫn ghi `status ENUM('pending','completed','cancelled')` (thiếu `'debt'`) và mô tả `confirmReceipt` update thẳng `status='completed'` (sai với code thật, xem state machine ở mục Services); không có mục nào cho bảng `purchase_order_payments`; không nhắc gì tới `PurchaseOrderPaymentService`/2 endpoint payments hay tính năng xuất PDF. Ngoài phạm vi commit này, Schema.md cũng đã lệch từ trước — `role` ENUM chưa có `'BranchManager'`.
...
**Docs — cần cập nhật:**
1. `Schema.md` §7 (`purchase_orders`) chưa phản ánh commit "Thêm nợ nhà cung cấp, xuất pdf hóa đơn": `status` ENUM thiếu `'debt'`, mô tả `confirmReceipt` sai (ghi update thẳng `'completed'`), thiếu hẳn mục cho bảng `purchase_order_payments` — xem "Known issues — tuần 5" #5.
