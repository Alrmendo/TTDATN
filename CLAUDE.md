...
7. ~~**WarehouseStaff xem/xác nhận được đơn nhập hàng và phiếu điều chuyển của chi nhánh khác**~~ — **đã fix (lỗ hổng phân quyền)**: phát hiện qua regression test (`test/TEST_RESULTS.md`, case #91/#92/#99 trong lần chạy 119-case gốc) — `PurchaseOrderService.getPurchaseOrderById`/`confirmReceipt` và `StockTransferService.confirmTransfer` trước đó không so `caller.storeId`/`callerStoreId` với chi nhánh của đơn/phiếu, dù `getPurchaseOrders`/`getTransfers` (list) đã store-scoped đúng từ trước — chỉ riêng đường "theo id" bị sót. Đã thêm check 403 ở cả 3 hàm.
